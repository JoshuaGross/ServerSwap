...
var server = function (deployMode) {
  ...
  var releaseResource = function (resource, requester) {
    ...
    if (waitingFor[resource].length > 1) return; // already freeing resource
  ...
  var resourceReady = function (resource) {
    ...
    console.log('Giving',resource,'to',requester.pid);
    ...
    var endResourceRace = function () {
      ...
      console.log('Killing late requester for',resource,tooLateRequester.pid);
  ...
  this.start = function () {
      ...
      var onData = function (data) {
        if (data.goAway) {
          if (data.goAway != process.pid) {
            console.log('ServerSwap daemon',data.goAway,'replacing',process.pid);
            process.exit();
          }
        } if (data.oldServer) {
          ...
          socket.on('error', function (e) {
            console.log('Error writing to old socket.',e);
          });
          ...
          socket.on('close', function (msg) {
            ...
            console.log('New server died unexpectedly:', msg, ' Servers left: ',newServers.length)
            closeAllSockets();

            if (deployMode && newServers.length === 0) {
              process.nextTick(function () {
                process.exit(0);
              });
            }
          ...
          socket.on('error', function (e) {
            console.log('Error writing to new socket.', e);
          });
          ...
          setTimeout(function () {
            if (data.waitingFor) {
              ...
              if (waitingFor[data.resourceFreed].freedBy.length >= oldServers.length) {
    ...
    var tryConnectingAsDaemon = function () {
      ...
      otherDaemon.on('error', function () {
        ...
        console.log('Succeeded daemonizing ServerSwap after',connectAttempts,'attempts.');
      });
    };
    ...
    var startServerPromotion = function () {
      promotionInterval = setInterval(function () {
        if (oldServers.length === 0 && newServers.length > 0) {
          console.log('ServerSwap: All old servers died; promoting', newServers.length, 'new server(s).')
          for (var i in newServers) {
...
var client = function () {
  var promoted = false, reconnectFailed = false, disconnectedAt;
  ...
  var resourceReady = function (resource) {
    // promote self when we start grabbing resources
    promoted = true;

    if (!waitingFor[resource] || waitingFor[resource].length == 0) return;
  ...
  var socketWrite = function (data) {
    if (serverSocket && serverSocket.readable && serverSocket.writable) {
  ...
  this.start = function () {
    var reconnectToServerSocket = function () {
      ...
      serverSocket.connect(serverPort, function () {
        ...
        for (var i in waitingFor) {
          if (waitingFor[i].length) {
            console.log('ServerSwap ['+process.pid+']: Re-requesting',i);
    ...
    var onData = function (data) {
      if (data.promote) {
        ...
        console.log('ServerSwap ['+process.pid+']: Too late, someone grabbed',data.tooLate,'while we were waiting. Race condition?');
        ...
        var onFreed = function () {
          console.log('ServerSwap ['+process.pid+']: Freed resource', data.freeResource);
          socketWrite(JSON.stringify({ resourceFreed: data.freeResource })+"\n");
        ...
        try {
          ...
          console.log('ServerSwap ['+process.pid+']: Error freeing resource', e);
          return onFreed();
        }
      }
    }
    ...
    setInterval(function () {
      ...
      if ((reconnectFailed || serverSocket === null) && (new Date() - disconnectedAt) > 250) {
        promoted = true;
      }
