      ...
      var onData = function (data) {
        if (data.goAway) {
          ...
          socket.on('close', function (msg) {
            ...
            setTimeout(function () {
              if (deployMode && newServers.length === 0 && oldServers.length === 0) {
                console.log('ServerSwap Server ['+process.pid+']: Deploy mode: all clients died. Quitting [old client timeout].')
                process.exit(0);
              }
            }, clientReconnectDelay*2);
          });
          ...
          socket.on('close', function (msg) {
            ...
            setTimeout(function () {
              if (deployMode && newServers.length === 0 && oldServers.length === 0) {
                console.log('ServerSwap Server ['+process.pid+']: Deploy mode: all clients died. Quitting [new client timeout].')
                process.exit(0);
              }
            }, clientReconnectDelay*2);
          });
