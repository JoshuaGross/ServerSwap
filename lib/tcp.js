...
var server = function (deployMode) {
  ...
  var closeAllSockets = function () {
    for (var i in oldServers) {
      ...
      process.nextTick(oldServers[i].end)
    }
    for (var i in newServers) {
      ...
      process.nextTick(newServers[i].end)
    }
    oldServers = [];
    newServers = [];
      ...
      var onData = function (data) {
        if (data.goAway) {
          ...
          socket.on('close', function (msg) {
            ...
            setTimeout(function () {
              if (deployMode && newServers.length === 0 && oldServers.length === 0) {
                console.log('ServerSwap Server ['+process.pid+']: all clients died in deploy mode. Quitting from old client timeout.')
                process.exit(0);
              }
          ...
          socket.on('close', function (msg) {
            ...
            console.log('ServerSwap Server ['+process.pid+']: New client died unexpectedly:', msg, ' New servers left: ',newServers.length, ' Old servers left: ',oldServers.length)

            setTimeout(function () {
              if (deployMode && newServers.length === 0 && oldServers.length === 0) {
                console.log('ServerSwap Server ['+process.pid+']: all clients died in deploy mode. Quitting from new client timeout.')
                process.exit(0);
              }
            }, clientReconnectDelay*2);
          });
...
var client = function () {
  ...
  this.start = function () {
    ...
    setInterval(function () {
      ...
      if ((reconnectFailed || serverSocket === null) && (new Date() - disconnectedAt) > clientReconnectDelay*2) {
