...
var server = function (deployMode) {
  ...
  var releaseResource = function (resource, requester) {
    ...
    console.log('ServerSwap Server ['+process.pid+']: Attempting to free resource', resource)
      ...
      var onData = function (data) {
        if (data.goAway) {
          if (data.goAway != process.pid) {
            console.log('ServerSwap Server ['+process.pid+']: replacing',process.pid)
            process.exit();
          }
        } if (data.oldServer) {
          ...
          socket.on('close', function (msg) {
            ...
            console.log('ServerSwap Server ['+process.pid+']: New client died unexpectedly:', msg, ' Servers left: ',newServers.length)
            closeAllSockets();
          ...
          console.log('ServerSwap Server ['+process.pid+']: Deploy Finished.')
          closeAllSockets();
          process.nextTick(function () {
            process.exit(0);
          });
