...
var server = function (deployMode) {
  ...
  var releaseResource = function (resource, requester) {
    ...
    for (var i in oldServers) {
      // when many servers are started simultaneously, sometimes a server will grab and use a resource by default; and then
      // another deploy server will attempt to free the resource /from/ the server /for/ the server
      if (oldServers[i].pid !== requester.pid) {
        console.log('ServerSwap Server ['+process.pid+']: Attempting to free resource', resource, 'from', oldServers[i].pid)
    ...
    var handleGoAway = function (socket, data) {
      if (data.goAway && data.goAway.pid != process.pid && (semver.gt(data.goAway.version, packageJson.version) || Date.parse(data.goAway.start) < startTime)) {
        if (!otherTcpServerRunning) {
          console.log('ServerSwap Server ['+process.pid+']: being replaced by',data.goAway, 'Our version:', packageJson.version, Date.parse(data.goAway.start) < startTime)
