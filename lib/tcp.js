  ...
  this.start = function () {
    var onNewConnection = function (socket) {
      ...
      var socketCloseResponse = function (msg) {
        ...
        if (respondToCloseEvents === false && !otherServersWaiting) return;

        var checkForDeployFailure = function () {
          console.log('ServerSwap Server ['+process.pid+']: check PIDs: ',watchPIDs);
    ...
    var handleGoAway = function (socket, data) {
      if (data.goAway && data.goAway.pid != process.pid && (semver.gt(data.goAway.version, packageJson.version) || Date.parse(data.goAway.start) < startTime)) {
        ...
        socket.write(JSON.stringify({ goAway: { pid: process.pid, version: packageJson.version, start: startTime } }));
        if (deployMode) {
          otherServersWaiting = true;
        }
      }
    };
